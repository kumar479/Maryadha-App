...
export default function MessagesScreen() {
  ...
  const loadChats = async () => {
    try {
      ...
      // Get chats where rep is assigned
      const { data: chatData, error: chatError } = await supabase
        ...
        `)
        .or(`order_id.eq.any(
          select id from orders where rep_id = '${rep.id}'
        ), sample_id.eq.any(
          select id from samples where rep_id = '${rep.id}'
        )`)
      ...
      const formattedChats = chatData?.map(chat => ({
        id: chat.id,
        brandName: chat.brands?.name || 'Unknown Brand',
  ...
  const handleChatPress = (chatId: string) => {
    router.push(`/rep/tabs/messages/${chatId}`);
  };
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={[Typography.body, styles.emptyText]}>
              No messages yet. Messages will appear here when you're assigned to orders or samples.
            </Text>
          </View>
        }
      />
